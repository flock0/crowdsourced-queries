 ...
 */
object QueryParser2 extends StandardTokenParsers {
  lexical.delimiters ++= List("(", ")", "\\", ".", "[", "]", ",", "<", ">", "=")
  lexical.reserved   ++= List("JOIN", "ON", "IN", "NOT", "INTERSECT", "UNION", "LIMIT", "ORDER BY", "DESC", "ASC", "WHERE", "FROM", "SUM", "DISTINCT", "NUMERIC", "AND", "OR", "True", "False")
  ...
  def parseSelectTree: Parser[SelectTree] = (
    "SELECT" ~ "(" ~ parseP ~ ")"~"FROM" ~ parseNl ^^ {case _~_~p~_~_~e => Select(e, p)}
 ...
 def regexStringLit(r: Regex): Parser[String] = acceptMatch( 
  ...
  val elem: Parser[String] = regexStringLit("[A-Za-z0-9_ ]+".r)
//  val int: Parser[String] = "[0-9]+".r
  val int: Parser[String] = regexStringLit("[0-9]+".r)
//  val nl: Parser[String] = "[a-zA-Z0-9- ]+".r
  val nl: Parser[String] = regexStringLit("[a-zA-Z0-9- ]+".r)
//  val str: Parser[String] = "[A-Za-z0-9_]+".r
  val str: Parser[String] = regexStringLit("[A-Za-z0-9_]+".r)
  
  def parseQuery(query: String): RootNode = {
	
	val tokens = new lexical.Scanner(query)
    phrase(parseRootNode)(tokens) match {
